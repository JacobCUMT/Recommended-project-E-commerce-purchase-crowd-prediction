...
--@exclude_input=tmall_rec_project.brand_cate2_dim
--@exclude_input=tmall_rec_project.dw_user_item_click_log
--odps sql 
--********************************************************************--
--author:aliyun5858408693
--create time:2024-11-06 14:22:24
--********************************************************************--
...
select t1.user_id, t1.brand_id,
    ...
    if(clt_item_90d is null, 0, clt_item_90d) as clt_item_90d,
...
from (
    ...
    from (
        ...
        from (
            select user_id, cate2_id, item_id, ds
        ...
        )t2 on t1.cate2_id=t2.cate2
    )t1
    group by user_id, brand_id
)t1 left join (
    select user_id, brand_id
         ,count(distinct if(ds>=to_char(dateadd(to_date(${bizdate}, 'yyyymmdd'),-90, 'dd'),'yyyymmdd'), item_id, null)) as clt_item_90d
         ,count(distinct if(ds>=to_char(dateadd(to_date(${bizdate}, 'yyyymmdd'),-60, 'dd'),'yyyymmdd'), item_id, null)) as clt_item_60d
         ,count(distinct if(ds>=to_char(dateadd(to_date(${bizdate}, 'yyyymmdd'),-30, 'dd'),'yyyymmdd'), item_id, null)) as clt_item_30d
         ,count(distinct if(ds>=to_char(dateadd(to_date(${bizdate}, 'yyyymmdd'),-15, 'dd'),'yyyymmdd'), item_id, null)) as clt_item_15d
         ,count(distinct if(ds>=to_char(dateadd(to_date(${bizdate}, 'yyyymmdd'),-7, 'dd'),'yyyymmdd'), item_id, null)) as clt_item_7d
         ,count(distinct if(ds>=to_char(dateadd(to_date(${bizdate}, 'yyyymmdd'),-3, 'dd'),'yyyymmdd'), item_id, null)) as clt_item_3d
         ,count(distinct if(ds>=to_char(dateadd(to_date(${bizdate}, 'yyyymmdd'),-1, 'dd'),'yyyymmdd'), item_id, null)) as clt_item_1d
    from (
        select t1.user_id, t2.brand_id, t1.item_id, t1.ds
        from (
            select user_id, cate2_id, item_id, ds
...
SELECT * FROM user_brand_cate2_cross_beh_feature_ads  WHERE ds='${bizdate}' LIMIT 100;
SELECT count(*) FROM user_brand_cate2_cross_beh_feature_ads WHERE ds='${bizdate}';